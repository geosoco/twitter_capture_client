...
from listeners import BaseListener, PrintingListener, RotatingFileListener
...
class Client(object):
	...
	def __init__(self, base_url, token,  ping_interval, update_interval, twitter_auth, output_config, source_addr = None):
		"""
		Client construction
		"""
	...
	def wait_for_job(self):
		"""
		polls for an active job assignment
		"""
		while running == True:
			...
			if active_jobs is not None:
				...
				if count > 0:
					...
					if count > 1:
						self.log.warn("more than one active_job. got %d for id %d", count, self.client_id )
	...
	def start_collection(self):
		...
		self.listener = RotatingFileListener(collection_name=collection_name, **self.output_config)
	...
	def run_collection(self):
		...
		# run while valid
		while running == True and archived_date == None:

			# get job status from server
			status_msg = self.sm.getStatus()

			# if we got null, there's a problem with the server, sleep and continue
			if status_msg is None:
				sleep(update_interval)
				continue

			# set up the status
			status = status_msg['status'] if 'status' in status_msg else STATUS_UNKNOWN
			keywords = [kw.strip() for kw in status_msg['twitter_keywords'].split(",")] if 'twitter_keywords' in status_msg else []
			...
			if self.stream and self.stream.isRunning():
				...
				#sm.putUpdate(listener.received, listener.total, listener.rate)
				...
				if last_update is None:
					...
					if delta.total_seconds() > self.update_interval:
						self.log.debug("update delta: %f", delta.total_seconds())
						do_update = True
					#else:
				...
				# update to server
				if do_update == True:
					self.sm.putUpdate(self.listener.received, self.listener.total, self.listener.rate)
	...
	def run(self):
		...
		while running == True:
			...
			if self.active_job is not None:
				self.log.info("working on job (id: %d, name: %s)", self.active_job_id, self.active_job["name"])
...
def run_capture( base_url, token, client_id, ping_interval, update_interval, twitter_auth, output_conf):
	sm = ServerMessenger(
		base_url = base_url,
		token = token,
		client_id = client_id
	...
	# create our streamer
	#listener = RotatingFileListener(**output_conf)
	...
	while running == True:
		status_msg = sm.getStatus()

		status = status_msg['status'] if 'status' in status_msg else STATUS_UNKNOWN
		keywords = [kw.strip() for kw in status_msg['twitter_keywords'].split(",")] if 'twitter_keywords' in status_msg else []

		log.debug("got status: %d", status_msg['status'])

		# check for collection change notice


		# are there any keyword changes?
		keywords_set = set(keywords)
		if keywords_set != current_keywords:
			log.info("twitter filter words changed: ")
			subtractions = current_keywords - keywords_set
			additions = keywords_set - current_keywords
		...
		if old_status != status:
			log.info("changing status %d -> %d", old_status, status)
			if status == STATUS_STARTING or status == STATUS_STARTED:
				if stream is not None and not stream.isRunning():
					log.info("Starting stream")
					stream.track_list = keywords
					listener.total = status_msg['total_count']
					stream.start()
					# ackknowledge that we have the newest keywords in here
					keywords_changed = False
			elif status == STATUS_STOPPING or status == STATUS_STOPPED:
		...
		if stream is not None:
			if stream.isRunning():
				log.debug("stream exists and is running")
				new_status = STATUS_STARTED
			else:
				log.debug("stream exists but is not running")
				new_status = STATUS_STOPPED
		else:
			new_status = STATUS_STOPPED
		...
		# if there's a discrepancy
		if new_status != status and new_status != STATUS_UNKNOWN:
			log.info("changing status %d -> %d", status, new_status)
			sm.updateStatus(new_status)
		...
		if stream and stream.isRunning():
			...
			#sm.putUpdate(listener.received, listener.total, listener.rate)
			sm.pingServer(listener.total, listener.rate)
			...
			if last_update is None:
				...
				if delta.total_seconds() > update_interval:
					log.debug("update delta: %f", delta.total_seconds())
					do_update = True
				#else:
					#log.debug("delta: %f", delta.total_seconds())
	...
	if stream is not None and stream.isRunning():
		...
		while stream and stream.isRunning():
			log.info("Waiting for logger to stop")
			sleep(1)
...
if __name__ == "__main__":
	...
	parser.add_argument("--configfile", default="config.json", help="config file to use")
	...
	if "logging" in config:
		...
		logging.getLogger("twitter_monitor.listener").setLevel(logging.getLevelName('WARN'))
